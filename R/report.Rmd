...
# input file
# https://abcs-amp.cancer.gov/uploads/external/312/384125a4200ff92e0f7bb9fe5ff9e3f2f6d14fb9
f <- paste0(root, '/data/HSL_VAL_HPV16VLP_Summary_V8.xlsx')
...
diff <- list(lloq_acon = 'Result', #'Result (Calculated Con.)',
             ...
             acc_acon = 'Calculated Result (EU/mL)', #'Result ((AU/mL))',
...
tables$sens %>%
    kable(digits = c(rep(1,4), 3), caption = paste0('Table ', which(names(tables) == 'sens'), ': Sensitivity data (LLOQ). Using the criteria that the lowest concentration for each sample was based on the Percent Error ≤50% and Within Dilution CV (replicates) ≤30%.'))
```

```{r uloq sensitivity table}
tables$sens2 %>%
    kable(digits = c(rep(1, 4), 2), caption = paste0('Table ', which(names(tables) == 'sens2'), ': Sensitivity data (ULOQ). Using the criteria that the highest concentration for each sample was based on the Percent Error ≤50% and Within Dilution CV (replicates) ≤30%.'))
```
