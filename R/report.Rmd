...
f <- paste0(root, '/data/HSL_VAL_HPV16VLP_Summary_V7_1.xlsx')
...
tables$sens %>%
    kable(digits = 1, caption = paste0('Table ', which(names(tables) == 'sens'), ': Sensitivity data (LLOQ). Using the criteria that the lowest concentration for each sample was based on the Percent Error ≤50% and Within Dilution CV (replicates) ≤30%.'))
```

```{r uloq sensitivity table}
tables$sens2 %>%
    kable(digits = 1, caption = paste0('Table ', which(names(tables) == 'sens2'), ': Sensitivity data (ULOQ). Using the criteria that the highest concentration for each sample was based on the Percent Error ≤50% and Within Dilution CV (replicates) ≤30%.'))
```
