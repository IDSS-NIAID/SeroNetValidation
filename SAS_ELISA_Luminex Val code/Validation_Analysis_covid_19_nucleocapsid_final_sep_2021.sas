...
%let file = COVID19_VAL_Nucleocapsid_Summary_Final_23SEP21;
...
data linear_range;
set all_linear;
if Assay ="Nucleocapsid IgG" and ( 0.12 > OD or OD > 2.9 ) then delete;
if Assay ="Nucleocapsid IgM" and ( 0.15 > OD or OD > 2.7 ) then delete;
run;
...
data Lin_2_4;
set Linear;
where assay="Nucleocapsid IgM" and sample_idd in ("LIN_2" "LIN_4");
if acon=. then delete;
run;
...
ods output Quantiles = qtls;
proc univariate data=lloq_sum2 normal;
class assay;
var concentration;
run;*/
...
data lloq_sum3 ;
set lloq_sum2 ;
if assay = "Nucleocapsid IgG" and concentration > 0.9824 then delete;
if assay = "Nucleocapsid IgM" and concentration > 0.6754 then delete;
run;
...
set lloq_uloq_sum;
if uloq <=3 then uloq=.;
if (Assay ="Nucleocapsid IgG" and sample_id = 'ULOQ2') or  (Assay ="Nucleocapsid IgG" and sample_id in ('LIN_4' 'LIN_2')) then lloq=.;
if (Assay ="Nucleocapsid IgG" and sample_id in ('ULOQ3' 'ULOQ4' 'LLOQ5')) or  (Assay ="Nucleocapsid IgM" and sample_id in ('LIN_1' 'LIN_3' 'LLOQ3' 'LLOQ4' 'LLOQ5' 'LLOQ7' 'LLOQ1' 'ULOQ7' 'ULOQ8')) then uloq=.;
run;
...
by assay;
model lg_acon =  Treatment;
random int/subject= SAmple_id;

contrast 'Freeze 10x vs 5x' 	Treatment 1 -1  0;
contrast 'Freeze 10x vs normal' Treatment 1  0 -1;
contrast 'Freeze 5x  vs normal' Treatment 0  1 -1;
...
if Treatment="" then Treatment="Normal";
if sample_id in ("High" "HIGH") then sample_id ="HIGH";
...
*by assay;
model lg_acon =  Treatment/s;
contrast 'Hemolysis High vs low' 	Treatment 1 -1  0;
contrast 'Hemolysis High vs normal' Treatment 1  0 -1;
contrast 'Hemolysis normal vs normal' 	Treatment 0  1 -1;
random int/subject=sample_id;
run;
